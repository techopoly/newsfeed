...
class Feed extends Component {
  ...
  loadPosts = direction => {
    ...
    fetch('http://localhost:8050/getPosts', {
      headers: {
      ...
      .then(res => {
        const resData = res.data.map(post => {
          return {
            ...post,
  ...
  finishEditHandler = postData => {
    ...
    let url = 'http://localhost:8050/createPost';
    let method = 'POST';
    if (this.state.editPost) {
      url = `http://localhost:8050/editPost?id=${this.state.editPost.id}`;
    }
    // const data = {
    ...
    fetch(url, {
      ...
      .then(resData => {
        ...
        const post = {
          ...
          content: resData.post.post_content,
          creator: resData.post.username,
  ...
  deletePostHandler = postId => {
    ...
    fetch(`http://localhost:8050/deletePost?id=${postId}`, {
      method: 'POST',
      headers: {
        Authorization: 'Bearer ' + this.props.token
      }
